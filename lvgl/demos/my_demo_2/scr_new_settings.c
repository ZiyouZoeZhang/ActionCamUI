...
#define INIT_NEW_ROLLER(name, list_array) \
    static settings_new_roller_t name = { \
        .ids = list_array, \
        .states_count = sizeof(list_array) / sizeof(list_array[0]), \
        .roller = NULL, \
        .state_label = NULL \
...
static const int list_new_auto_poweroff[] = {STRING_OFF,STRING_1MIN, STRING_3MIN, STRING_5MIN};
...
INIT_NEW_ROLLER(auto_poweroff, list_new_auto_poweroff);
...
const char* get_roller_text(settings_roller_t* roller, int index) {
    return lv_lang_string[roller->ids[index]][g_current_lang];
}
